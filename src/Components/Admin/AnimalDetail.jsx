...
class AnimalDetail extends Component {
    ...
    handleMinWeightChange = (e) =>{
        this.setState({activeShippingFee:{...this.state.activeShippingFee,
            "animal": this.state.animalDetail.animal_ID,
    ...
    handleSubmitShippingFeeForm = (e) =>{
        e.preventDefault()
        var url = `http://50.116.29.247/add/animal/product/${this.state.activeShippingFee.animal}/shipping-fee/`
        var csrfToken = this.getCookie('csrftoken')
        var method_allowed = "POST"
        var alertMessage = "Shipping fee has been added"

        if (this.state.editingShippingFee){
            url=`http://50.116.29.247/update/animal/product/shipping-fee/${this.state.activeShippingFee.shippingfee_ID}/`
            method_allowed = "PUT"
    ...
    deleteShippingFee = (activeShippingFeeItem) =>{
        var url = `http://50.116.29.247/delete/animal/product/shipping-fee/${activeShippingFeeItem.shippingfee_ID}/`
        var csrfToken = this.getCookie('csrftoken')
    ...
    handleChangeOnCountry = (e) =>{
        this.setState({activeShippingPrice: {...this.state.activeShippingPrice,
            animal_shipping_fee: this.state.activeShippingFee.shippingfee_ID,
    ...
    handleSubmitOnCountryPriceForm = (e) =>{
        ...
        var url = `http://50.116.29.247/animal/product/shipping-fee/${this.state.activeShippingPrice.animal_shipping_fee}/add/country-price/`
        var csrfToken = this.getCookie('csrftoken') 
        var method_allowed = "POST"
        var alertMessage = "Price has been added! close the window and view prices again"

        if (this.state.editingShippingPrice){
            url = `http://50.116.29.247/animal/product/shipping-fee/update/country-price/${this.state.activeShippingPrice.shippingcountryprice_ID}/`
            method_allowed = "PUT"
    ...
    deleteShippingPrice = (activeShippingPriceItem) =>{
        var url = `http://50.116.29.247/animal/product/shipping-fee/delete/country-price/${activeShippingPriceItem.shippingcountryprice_ID}/`
        var csrfToken = this.getCookie('csrftoken')
    ...
    render() { 
        var animal = this.state.animalDetail
                                ...
                                <React.Fragment>
                                    <div className="bind-product-image-and-specifications">
                                        <div className="product-image">
                                            <img src={"http://50.116.29.247" + animal.image} alt="animal"/>
                                        </div>
                                        <div className="product-specifications single-coffee-detail-description">
                                            <h5>{`${animal.name}`}</h5>
                                            <h6>{`price : ${animal.currency} ${animal.price} `}</h6>
                                            <br/>
                                            {animal.animal_shipping_fees.length !== 0 ?
                                            <React.Fragment>
                                                <p className="text-dark mb-1 text-center font-weight-bold">{`${animal.name}  shipping fee list`}</p>
                                                <table className="table table-bordered" style={{whiteSpace: "nowrap"}}>
                                                    ...
                                                    <tbody>
                                                    {   
                                                        animal.animal_shipping_fees.map(shipping_fee =>(
            ...
            >
                {
                    props => (
                        <div className="shipping-price-div" style={props}>
                            ...
                            {this.state.activeShippingFee.animalshippingcountryprices.length !== 0 ?
                            <React.Fragment>
                                <table className="table table-bordered" style={{whiteSpace: "nowrap"}}>
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                    { this.state.activeShippingFee.animalshippingcountryprices.map(country_price =>(
