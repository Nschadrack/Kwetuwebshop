...
class CustomerSpecialOrderDetail extends Component {
    state = { 
        orderdetatil:{}
     }
                            ...
                            <React.Fragment>
                                {
                                    this.state.orderdetatil.ordered_products.map(item=>
                                        <tr key={item.item_number}>
                                            ...
                                            <td style={{paddingLeft: "30px"}}>{Number(item.quantity).toFixed(2)}</td>
                                            <td style={{paddingLeft: "30px"}}>{item.unit}</td>
                                            <td style={{paddingLeft: "20px"}}>{`$ ${(Number(item.price) * Number(item.quantity)).toFixed(2)}`}</td>
                                            <td className="action-buttons">
                                                <Link className="order-detail-view-btn"
                                                to={`/${this.props.activeUser.first_name}-${this.props.activeUser.last_name}/account/special-order/product/${item.item_number}/detail`}><FontAwesomeIcon icon={faEye}/> view</Link>
                                            </td>
                                        </tr>
                                    )
                                }
